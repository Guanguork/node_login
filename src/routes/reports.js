...
router.get('/reports/add', isAuthenticated, (req, res, next) => {
...
router.put('/reports/edit-report/:id', isAuthenticated, async (req, res) => {
    const {mounth, year, tasks} = req.body
    await Report.findByIdAndUpdate(req.params.id, {mounth, year, tasks})
...
router.post('/reports/new-report', isAuthenticated, async (req, res, next) => {
    ...
    if(errors.length > 0) {
        ...
        const newReport = new Report({mounth, year, tasks})
